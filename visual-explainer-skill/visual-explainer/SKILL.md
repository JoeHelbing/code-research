...
Detailed prompt templates live in `./commands/`. Read the relevant template before generating output for that workflow — each one specifies the data to gather, the structure to use, and the rendering approach.

| Workflow | Template | What it does |
|----------|----------|--------------|
| Web diagram | `commands/generate-web-diagram.md` | Generate an HTML diagram for any topic |
| Visual plan | `commands/generate-visual-plan.md` | Generate a visual implementation plan for a feature |
| Slides | `commands/generate-slides.md` | Generate a magazine-quality slide deck |
| Diff review | `commands/diff-review.md` | Visual diff review with architecture comparison and code review |
| Plan review | `commands/plan-review.md` | Compare a plan against the codebase with risk assessment |
| Project recap | `commands/project-recap.md` | Mental model snapshot for context-switching back to a project |
| Fact check | `commands/fact-check.md` | Verify accuracy of a document against actual code |
| Share page | `commands/share-page.md` | Deploy an HTML page to Vercel and get a live URL |
