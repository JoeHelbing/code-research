...
Render syntax-highlighted output with text nodes. Source text, repository
content, and control values are untrusted; never interpolate them into
`innerHTML`.

```javascript
function renderPreview(sql) {
  const preview = document.getElementById("preview");
  const fragment = document.createDocumentFragment();
  const tokenPattern = /('(?:''|[^'])*'|\b(?:SELECT|FROM|WHERE|JOIN|ON|GROUP BY|ORDER BY|LIMIT)\b|\b(?:users|orders|products)\b)/gi;

  for (const token of sql.split(tokenPattern)) {
    if (!token) continue;

    const span = document.createElement("span");
    if (/^(?:SELECT|FROM|WHERE|JOIN|ON|GROUP BY|ORDER BY|LIMIT)$/i.test(token)) {
      span.className = "kw";
    } else if (/^(?:users|orders|products)$/i.test(token)) {
      span.className = "tbl";
