...
- Highlight suggested lines with a semantic status border and visible label
- Show pending, approved, and rejected states through text plus a visual token;
...
Define status tokens in both theme blocks using values derived from the selected
examples. Pair every color treatment with a text label or status icon.

```css
.doc-line.has-suggestion {
  border-left: 3px solid var(--review-pending);
