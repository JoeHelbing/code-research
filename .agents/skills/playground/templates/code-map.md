...
This template defines behavior and data shape only. Read
...
| Connection types | Checkboxes with line indicators | Data flow, tool calls, events |
...
Define 3-5 connection types with distinct semantic roles. Resolve their colors
from the selected examples, and distinguish them with labels and stroke patterns
so color is never the only signal.

| Type | Semantic token | Stroke | Use for |
| --- | --- | --- | --- |
| `data-flow` | `--connection-data` | Solid | Request/response and data passing |
| `tool-call` | `--connection-tool` | Dashed | Function calls and API invocations |
| `event` | `--connection-event` | Short dash | Async events and pub/sub |
| `skill-invoke` | `--connection-skill` | Long dash | Plugin or skill activation |
| `dependency` | `--connection-dependency` | Dotted | Import and require relationships |

Use semantic tokens in SVG markers:

```html
<marker id="arrowhead-data" markerWidth="8" markerHeight="6"
        refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="var(--connection-data)"/>
</marker>
```
...
Map each layer to a semantic role derived from the selected examples. Define
both dark and light token values through `anthropic-style.md`; do not place
literal palette values in node data. Keep layer names visible on nodes and in
the legend so the grouping remains understandable without color.
